...
document.getElementById("exportDOCX").addEventListener("click", function() {
    ...
    try {
        ...
        // Build HTML string that html-docx-js will convert
        let content = `
        <html><head><meta charset="UTF-8">
        <style>
          body { font-family: Calibri, Arial, sans-serif; font-size: 11pt; color: #111; }
          h1 { font-size: 20pt; color: #2A4D8F; border-bottom: 2px solid #2A4D8F; padding-bottom: 6px; }
          h2 { font-size: 14pt; color: #1f3a6b; margin-top: 24pt; }
          p { font-size: 11pt; line-height: 1.5; }
          ol { margin: 0; padding-left: 20pt; }
          li { font-size: 13pt; font-weight: bold; margin-bottom: 8pt; }
          .desc { font-size: 11pt; font-weight: normal; color: #444; margin: 4pt 0 10pt 0; }
          .caption { font-size: 9pt; color: #666; font-style: italic; }
          .after { font-size: 11pt; color: #333; margin: 4pt 0; }
          .ending { font-size: 12pt; color: #2A4D8F; font-weight: bold; border-top: 1px solid #2A4D8F; margin-top: 24pt; padding-top: 8pt; }
          img { max-width: 100%; }
        </style>
        </head><body>`;
        ...
        steps.forEach(function(step) {
            ...
            if (stepLong.trim()) {
                content += `<p class="desc">${stepLong.replace(/\n/g,'<br>')}</p>`;
            }
            ...
            imgBlocks.forEach(function(block) {
                ...
                photoCounter++;
                const sizeW = img.style.width || "100%";
                const fotLabel = prefix
                    ? (caption ? (prefix + " " + photoCounter + ": " + caption) : (prefix + " " + photoCounter))
                    : caption;

                content += `<p><img src="${img.src}" style="width:${sizeW};max-width:14cm;"></p>`;
                if (fotLabel) content += `<p class="caption">${fotLabel}</p>`;
                if (after.trim()) content += `<p class="after">${after.replace(/\n/g,'<br>')}</p>`;
            });
        });
        content += '</ol>';
        ...
        // Ending
        if (ending.trim()) {
            content += `<p class="ending">${ending.replace(/\n/g,'<br>')}</p>`;
        }
        ...
        saveAs(converted, 'instrukcja.docx');
